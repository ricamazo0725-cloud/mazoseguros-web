      ...
      <div className="grid lg:grid-cols-2 gap-12">
        <div>
          ...
          <div className="space-y-4 mb-8">
            ...
            <a href={mapsUrl} target="_blank" rel="noreferrer" className="btn-secondary inline-block ml-2">
              📍 Ver en Google Maps
            </a>
          </div>
        ...
        <form onSubmit={handleSubmit} className="doc-card p-8 space-y-4">
          ...
          <button type="submit" disabled={status === "sending"} className="btn-cta w-full disabled:opacity-50">
            {/* Arreglado: en apps/web esta línea tenía un typo (le faltaba el "?" del
                ternario) que dejaba el botón siempre con el texto "Enviando…" cortado. */}
            {status === "sending" ? "Enviando…" : "Enviar solicitud"}
          </button>

          {status === "sent" && (
            <p className="text-xs text-accent font-mono">Solicitud enviada. Te contactaremos pronto.</p>
          )}
          {status === "error" && (
            <p className="text-xs text-danger font-mono">No se pudo enviar. Escríbenos por WhatsApp.</p>
          )}
        </form>
      </div>
    </section>
