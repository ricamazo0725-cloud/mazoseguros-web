  ...
  return (
    <section className="max-w-6xl mx-auto px-6 py-24">
      <div className="flex items-end justify-between mb-12 flex-wrap gap-4">
        ...
        <Link href="/blog" className="text-sm font-semibold text-accent hover:opacity-80 focus-ring rounded">
          Ver todo el blog →
        </Link>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        {posts.slice(0, 3).map((post) => (
          <Link
            key={post.id}
            href={`/blog/${post.slug}`}
            className="doc-card overflow-hidden group focus-ring block"
          >
            {post.cover_image && (
